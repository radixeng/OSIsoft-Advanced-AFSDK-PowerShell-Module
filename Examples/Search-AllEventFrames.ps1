...
# $OutputFileDir = "C:\Scripts\New-CsvOfOrphanEventFrames\output"
$OutputFileName = "OrphanEventFrames_$PI_SYSTEM_NAME_$Today.csv"
...
function Get-OpenAFEventFrames {
    ...
    switch ($PSCmdlet.ParameterSetName)
    {
        ...
        'AfDatabase' {
            ...
            $searchTokens.Add([OSIsoft.AF.Search.AFSearchToken]::new(
                [OSIsoft.AF.Search.AFSearchFilter]::Analysis,
                [OSIsoft.AF.Search.AFSearchOperator]::Equal,
                $null
            )) | Out-Null

            # $searchTokens.Add([OSIsoft.AF.Search.AFSearchToken]::new(
            #     [OSIsoft.AF.Search.AFSearchFilter]::ElementName,
            #     [OSIsoft.AF.Search.AFSearchOperator]::Equal,
            #     $null
            # )) | Out-Null
...
# $afdatabase = $afserver.databases |where-object {$_.name -eq "Vacuum Coaters"}

$AFOrphanEventFrames = Get-OpenAfEventFrames -AfServer $afserver

$AfOrphanEventFramesPS = Convert-ObjectsToPsCustomObjects $AfOrphanEventFrames


# $aforphaneventframesPS | Export-Csv -Path $OutputFilePath
[PSCustomObject[]]$AfOrphanEventFramesPS | Out-GridView
